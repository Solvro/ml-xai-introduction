...
def _ensure_mnist_available(root: Path) -> None:
    ensure_dataset_available(root, "MNIST", datasets.MNIST, MNIST_RAW_FILES, "MNIST")
...
def get_mnist_datasets(
    ...
    transform = build_transform(0.1307, 0.3081, normalize=normalize)
...
def build_mnist(cfg: DictConfig) -> DataBundle:
    ...
    transform = build_transform(0.1307, 0.3081, normalize=bool(cfg.data.normalize))
